...
async function crearReserva(payload) {
  ...
  return sequelize.transaction(async (t) => {
    ...
    for (const item of habitaciones) {
      ...
      if (!habitacion) throw crearError(`Habitación no existe: ${item.habitacion_id}`, 404);
    ...
    for (const s of servicios) {
      ...
      if (!servicio) throw crearError('Servicio no existe', 404);
      if (String(servicio.hotel_id) !== String(hotel_id)) {
    ...
    for (const d of detallesCrear) {
      await models.DetalleReserva.create({ reserva_id: reserva.id, ...d }, { transaction: t });
    }

    for (const sr of serviciosCrear) {
      await models.ServicioReserva.create({ reserva_id: reserva.id, ...sr }, { transaction: t });
    }
...
async function listarReservas(filtros = {}) {
  ...
  return models.Reserva.findAll({
    ...
    include: [
      ...
      { model: models.DetalleReserva, as: 'detalles', include: [{ model: models.Habitacion, as: 'habitacion' }] },
      { model: models.Pago, as: 'pagos' },
      { model: models.ServicioReserva, as: 'servicios', include: [{ model: models.Servicio, as: 'servicio' }] },
      { model: models.Valoracion, as: 'valoracion' },
    ],
  });
}
